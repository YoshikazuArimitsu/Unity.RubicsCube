...
/// </summary>
public class HandDraggableMarker : MonoBehaviour,
                             IFocusable,
                             IInputHandler,
                             ISourceStateHandler {
    ...
    private SurfaceController getRayHitSurface() {
        Vector3 vecMarker = transform.position - GazeManager.Instance.GazeOrigin;
        Ray ray = new Ray(GazeManager.Instance.GazeOrigin, vecMarker);
        RaycastHit hit;

        if(Physics.Raycast(ray, out hit)) {
            //var hitTrans = hit.collider.GetComponent<Transform>();
            //Debug.LogFormat("Raycast hit transform : {0}", hitTrans.name);

            return hit.collider.GetComponent<SurfaceController>();
        }
        return null;
    }
    ...
    private void postDragging() {
        //Debug.LogFormat("postDragging : {0}", transform.position);
        SurfaceController sc = getRayHitSurface();
        if (sc != null) {
            sc.InputOver();
        }
